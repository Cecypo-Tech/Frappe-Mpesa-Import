...
Design notes
------------
* The target doctype, ``Mpesa C2B Payment Register`` (owned by the
  ``frappe_mpsa_payments`` app), has **no unique index on ``transid``**.
  Deduplication is therefore entirely application-level and lives here.
...
def import_statement(parsed, source_name: str) -> dict:
	...
	:param parsed: a ``ParsedStatement`` from ``frappe_mpesa_import.statement_parser``
...
def _process_row(parsed, row, position: int) -> dict:
	...
	try:
		...
		existing = frappe.db.get_value(TARGET_DOCTYPE, {"transid": row.receipt_no}, "name")
		if existing:
		...
		express = frappe.db.get_value(EXPRESS_DOCTYPE, {"transaction_id": row.receipt_no}, "name")
		if express:
			outcome["status"] = STATUS_BLOCKED
			outcome["reference"] = express
			outcome["reason"] = f"Transaction already captured by {EXPRESS_DOCTYPE} {express}"
			return outcome
