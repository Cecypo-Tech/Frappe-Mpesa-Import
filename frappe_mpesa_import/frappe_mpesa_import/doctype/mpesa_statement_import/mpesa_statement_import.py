...
from frappe_mpesa_import import importer
from frappe_mpesa_import.statement_parser import StatementParseError, parse_statement
...
class MpesaStatementImport(Document):
	...
	def _get_file_path(self) -> str:
		...
		if not file_doc:
			frappe.throw(_("Could not find the uploaded file record for {0}.").format(self.statement_file))
		...
		try:
			...
			frappe.throw(_("The attached statement file could not be read from disk: {0}").format(str(e)))
	...
	def _parse(self, file_path: str):
		try:
			...
			frappe.throw(
				_("This file could not be read as an M-Pesa statement: {0}").format(str(e)),
	...
	def _throw_already_imported(self, earlier: str | None = None):
		...
		if earlier:
			frappe.throw(
				_("This exact statement file was already uploaded as {0}. Open that import instead.").format(
					get_link_to_form("Mpesa Statement Import", earlier)
				),
