"""Unit tests for :mod:`frappe_mpesa_import.statement_parser`.

These are plain pytest tests -- the parser must not import ``frappe``, so no
site or bench context is needed::

    /home/kushal/frappe-bench/env/bin/python -m pytest \
        apps/frappe_mpesa_import/frappe_mpesa_import/tests/
...
from frappe_mpesa_import.statement_parser import (
	ParsedStatement,
	StatementParseError,
	StatementRow,
	_parse_sheet,
...
from frappe_mpesa_import.tests.fixtures import make_fixture
...
def test_parser_module_does_not_import_frappe():
	...
	from frappe_mpesa_import import statement_parser
